...
function App() {
	...
	const CombinedChunkGrid = ({ regions }) => {
		...
		return (
			<div className="grid grid-cols-128 gap-0 w-[512px] h-[512px] border border-base-300 rounded-lg overflow-hidden">
				{grid.map((row, z) =>
					row.map((chunk, x) => (
						<div
							key={`${x}-${z}`}
							className={`w-1 h-1 transition-colors duration-200 ${
								chunk
