...
function App() {
	...
	async function analyzeRegion(filePath) {
		...
		setAnalysis(result);
		console.log("Full analysis result:", result); // Log the full result to the console
	}
				...
				<div>
					...
					<ul>
						{analysis.chunks.slice(0, 10).map((chunk, index) => (
							<li key={index}>
								Chunk ({chunk.x}, {chunk.z}, {chunk.y}): Status - {chunk.status}
								, Data Version - {chunk.data_version}, Last Update -{" "}
								{new Date(chunk.last_update).toLocaleString()}, Inhabited Time -{" "}
